...
public class CustomFunctionsStock {

    // ---------------------------------------------------------------------
 
    @Inject
    public CustomFunctionsStock() {
    }

    // ---------------------------------------------------------------------
 
    /**
     * Returns a stock node by it's name property.
     *
     * @return node object
     * throws RepositoryException when stock is not found.
     */
    public Node getStockNode(String name) throws RepositoryException {
        ...
        try { 
            ...
            Query query = queryManager.createQuery("select * from [stock] WHERE NAME() = '" + name + "'", "JCR-SQL2");
     ...
     * @return string
     */ 
    public List<Node> getStockArticles(int perPage, int offset, String stockUuid) throws RepositoryException {
